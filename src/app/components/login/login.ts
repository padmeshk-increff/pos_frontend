    ...
    this.authService.login(this.form)
      ...
      .subscribe({
        ...
        error: (err: HttpErrorResponse) => {
          // Special handling for 401 (unauthorized) - show specific login error message
          if (err.status === 401) {
            const message = err.error?.message || 'Invalid email or password. Please check your credentials.';
            // Check if message is HTML
            if (typeof message === 'string' && (message.trim().startsWith('<!DOCTYPE') || message.trim().startsWith('<html'))) {
          ...
          handleHttpError(err, this.toastService, 'Login failed. Please try again.');
        }
      });
  }
}
